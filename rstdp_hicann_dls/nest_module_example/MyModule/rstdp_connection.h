...
#ifndef RSTDP_CONNECTION_H
#define RSTDP_CONNECTION_H
...
class RSTDPConnection : public Connection< targetidentifierT >
{
...
public:
   ...
   */
  RSTDPConnection( const RSTDPConnection& );
...
template < typename targetidentifierT >
inline void
RSTDPConnection< targetidentifierT >::send( Event& e,
  thread t,
  double_t t_lastspike,
  const CommonSynapseProperties& )
{
  ...
  while ( start != finish )
  {
    ...
    if ( minus_dt == 0 )
      continue;
    accumulation_ = facilitate_( accumulation_, Kplus_ * std::exp( minus_dt / tau_plus_ ) );
  }

  // depression due to new pre-synaptic spike
  accumulation_ = depress_( accumulation_, target->get_K_value( t_spike - dendritic_delay ) );
...
template < typename targetidentifierT >
RSTDPConnection< targetidentifierT >::RSTDPConnection()
  : ConnectionBase()
  , weight_( 1.0 )
  , tau_plus_( 20.0 )
  , lambda_( 0.01 )
  ...
  , Wmax_( 100.0 )
  , Kplus_( 0.0 )
  , accumulation_( weight_ )
{
}

template < typename targetidentifierT >
RSTDPConnection< targetidentifierT >::RSTDPConnection(
  const RSTDPConnection< targetidentifierT >& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
  ...
  , Wmax_( rhs.Wmax_ )
  , Kplus_( rhs.Kplus_ )
  , accumulation_( rhs.weight_ )
{
}

template < typename targetidentifierT >
void
RSTDPConnection< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
  ConnectionBase::get_status( d );
  def< double_t >( d, names::weight, weight_ );
  def< double_t >( d, "accumulation", accumulation_ );
...
template < typename targetidentifierT >
void
RSTDPConnection< targetidentifierT >::set_status( const DictionaryDatum& d, ConnectorModel& cm )
{
  ConnectionBase::set_status( d, cm );
  updateValue< double_t >( d, names::weight, weight_ );
  updateValue< double_t >( d, names::weight, accumulation_ );
...
#endif // of #ifndef RSTDP_CONNECTION_H
