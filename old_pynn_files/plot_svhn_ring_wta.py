...
#from svhn_ring_wta import num,tstop
from SEM_ring_wta import num,tshow,tpaus
tstop = tshow+tpaus
...
color_input = ('red','blue')
color_exc = ('green','red')
color_inh = ('blue','green')
...
def plot_section(ax1,offset,num_neurons,file_leaf,color,plot_rates,skiprows):
    ...
    for j in range(num_neurons):
        ax1.plot(spikes[j],np.ones(len(spikes[j]))*(offset+j),linestyle='',marker='|',color=color[0])

    ax2 = ax1.twinx()
    ax2.plot(bins,rates,color=color[1])
...
if 1:
    ...
    plot_section(ax3,offset,num['l1_exc_neurons'],'exc_1',color_exc, plot_rates, 0)

    ax4 = plt.subplot(515,sharex=ax0)
    plot_section(ax4,offset,num['l1_inh_neurons'],'inh_1',color_inh, plot_rates, 0)
